...
int main() {
	...
	float vertices2[] = {
	 300.0f , 100.0f,	-1.0f,		 1.0f , 0.5f, 0.3f , 0.5f,
	...
	glm::mat4 model = glm::translate(glm::mat4(1.0f), translation); // represents camera , move the whole world in reverse direction of your movement
	...
	while (!glfwWindowShouldClose(window)) {
		...
		mvp = proj * view * model;
		UniformShader.setUniformMat4f("u_MVP", mvp);
		UniformShader.SetUniform4fv("u_Color", colors);
		//UniformShader.SetUniform4f("u_Color", r, g, b, 0.7f);
		//UniformShader.SetUniform4f("u_Color", 1.0f, 0.0f, 0.0f, 0.7f);
		renderer.Draw(va1, ib, UniformShader);

		// draw right tri
		//UniformShader.SetUniform4f("u_Color", g, b, r, 0.7f);
		//UniformShader.SetUniform4f("u_Color", 0.0f, 0.0f, 1.0f, 0.7f);
	...
	/*	TextureShader.Bind();
		...
		myImGui.debugV3Input(translation , "Translation" , 0 , width * 1.0);
		ImGui::ColorEdit4("clear color", (float*)&colors);
		ImGui::End();
		myImGui.EndFrame();
